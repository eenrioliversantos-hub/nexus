...
const TemplateDetailPage: React.FC<TemplateDetailPageProps> = ({ template, onBack, onUseTemplate }) => {
    return (
        <div className="flex flex-col h-screen bg-background text-text-primary">
            ...
            <main className="flex-1 overflow-y-auto p-6 lg:p-8">
                 <p className="text-center text-text-secondary max-w-3xl mx-auto mb-8">{template.description}</p>
                 <Tabs defaultValue="overview" className="max-w-5xl mx-auto">
                    ...
                    <TabsContent value="overview" className="mt-6">
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                               <DetailItem label="Objetivo">{template.systemOverview.objective}</DetailItem>
                               <DetailItem label="Usuários Alvo">{template.systemOverview.targetUsers}</DetailItem>
                               <DetailItem label="Principais Funcionalidades">
                                   <ul className="list-disc list-inside space-y-1 mt-2">
                                       {template.systemOverview.mainFeatures.map(f => <li key={f}>{f}</li>)}
                                   </ul>
                               </DetailItem>
                               <DetailItem label="Requisitos Não-Funcionais">
                                   <ul className="list-disc list-inside space-y-1 mt-2">
                                       {template.systemOverview.nonFunctionalRequirements.map(r => <li key={r}>{r}</li>)}
                                   </ul>
                               </DetailItem>
                            </CardContent>
                        </Card>
                    ...
                    <TabsContent value="users" className="mt-6">
                        <div className="space-y-4">
                            {template.userProfiles.map(profile => (
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <DetailItem label="Frontend">
                                    <div className="flex flex-wrap gap-2 mt-2">{template.technologyStack.frontend.map(t => <Badge key={t}>{t}</Badge>)}</div>
                                </DetailItem>
                                 <DetailItem label="Backend">
                                    <div className="flex flex-wrap gap-2 mt-2">{template.technologyStack.backend.map(t => <Badge key={t}>{t}</Badge>)}</div>
                                </DetailItem>
                                 <DetailItem label="Banco de Dados">
                                    <div className="flex flex-wrap gap-2 mt-2">{template.technologyStack.database.map(t => <Badge key={t}>{t}</Badge>)}</div>
                                </DetailItem>
                                 <DetailItem label="DevOps">
                                    <div className="flex flex-wrap gap-2 mt-2">{template.technologyStack.devops.map(t => <Badge key={t}>{t}</Badge>)}</div>
                                </DetailItem>
                            </CardContent>
                        </Card>
                    </TabsContent>
