...
const SupportTicketsPage: React.FC<SupportTicketsPageProps> = ({ tickets, onCreateTicket }) => {
    const [isCreatingTicket, setIsCreatingTicket] = useState(false);
    const [newTicket, setNewTicket] = useState({ subject: '', priority: 'Média' as 'Baixa' | 'Média' | 'Alta', description: '' });

    const handleSendTicket = () => {
        if (newTicket.subject && newTicket.description) {
            onCreateTicket(newTicket);
            setIsCreatingTicket(false);
            setNewTicket({ subject: '', priority: 'Média', description: '' }); // Reset form
        } else {
                ...
                <DialogHeader>
                 ...
                 <DialogContent className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <Input id="subject" placeholder="Ex: Erro ao gerar fatura" value={newTicket.subject} onChange={(e) => setNewTicket(p => ({...p, subject: e.target.value}))} />
                    </div>
                     <div className="space-y-2">
                        <Label htmlFor="priority">Prioridade</Label>
                        <Select value={newTicket.priority} onValueChange={(v: any) => setNewTicket(p => ({...p, priority: v}))}>
                     ...
                     <div className="space-y-2">
                        ...
                        <Textarea id="description" placeholder="Descreva o problema em detalhes..." value={newTicket.description} onChange={(e) => setNewTicket(p => ({...p, description: e.target.value}))} />
                    </div>
                      <div className="space-y-2">
                 ...
                 <DialogFooter>
                    ...
                    <Button onClick={handleSendTicket}>Enviar Ticket</Button>
                </DialogFooter>
            </Dialog>
