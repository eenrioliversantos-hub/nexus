...
const ClientContractDetailPage: React.FC<ClientContractDetailPageProps> = ({ contract, invoices, onBack, onSign }) => {
    return (
            ...
            <header className="bg-sidebar/80 backdrop-blur-sm border-b border-card-border px-6 py-4 flex items-center justify-between flex-shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-text-secondary">ID: {contract.id} | Projeto: {contract.projectName}</p>
                    </div>
                </div>
                     ...
                     {contract.status === 'pending' && (
                        <Card className="mt-8 bg-accent/10 border-accent/30">
                            <CardHeader>
                                ...
                                <CardDescription>Este contrato está aguardando sua assinatura para que o projeto possa prosseguir.</CardDescription>
                            </CardHeader>
                            <CardFooter className="flex justify-end">
                                <Button size="lg" onClick={() => onSign(contract.id)}>
                                    <Icon name="pen-tool" className="h-5 w-5 mr-2" />
                                    Assinar Contrato
                                </Button>
                            </CardFooter>
                        </Card>
